...
function initAnalyticsDashboard() {
  ...
  const providers = window.StoryAnalytics?.providers || { ga4: false, metaPixel: false };
  const connected = Boolean(providers.ga4 || providers.metaPixel);
  const providerStatus = `GA4 ${providers.ga4 ? "✓" : "—"} · Meta Pixel ${providers.metaPixel ? "✓" : "—"}`;
  state.classList.toggle("connected", connected);
  state.innerHTML = `<span>${connected ? "●" : "○"}</span><div><b>${esc(connected ? t("analyticsConnected") : t("analyticsNotConnected"))}</b><p>${esc(connected ? t("analyticsActive") : t("analyticsSetup"))}</p><small>${esc(providerStatus)}</small></div>`;
